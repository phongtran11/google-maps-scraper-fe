...
type GroupedSelectCheckboxProps = {
  "aria-label"?: string;
  className?: string;
  disabled?: boolean;
  groups: GroupOption[];
              ...
              <div className="flex h-full flex-1 flex-col overflow-y-auto p-3">
                {activeGroup ? (
                  <div className="flex flex-col gap-3">
                    {/* Select All option */}
                    <label className="mb-2 flex w-full cursor-pointer items-center gap-2 font-medium">
                      <Checkbox
                        checked={isIndeterminateInActive ? "indeterminate" : isAllSelectedInActive}
                        onCheckedChange={handleToggleSelectAllActive}
                      />
                      <span className="text-sm leading-none font-medium">Chọn tất cả</span>
                    </label>
                    ...
                    <div className="flex flex-col">
                      {activeGroup.options.map((opt) => (
                        <div
                          className="hover:bg-muted -mx-2 flex items-center px-4 py-2 transition-colors"
                          key={opt.key}
                        >
                          <label className="flex w-full cursor-pointer items-center gap-2">
                            <Checkbox
                              checked={draftSelected.includes(opt.key)}
                              disabled={opt.disabled}
                              onCheckedChange={() => handleToggleOption(opt.key)}
                            />
